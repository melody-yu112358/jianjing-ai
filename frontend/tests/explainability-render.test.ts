...
const dialogPath=new URL(`./render-dialog-${process.pid}.mjs`,import.meta.url);
writeFileSync(dialogPath,transpileModule(readFileSync(new URL('../components/presentation-dialog.tsx',import.meta.url),'utf8'),{compilerOptions:{module:ModuleKind.ESNext,jsx:JsxEmit.ReactJSX}}).outputText);
const source=readFileSync(new URL('../components/roadshow-insights.tsx',import.meta.url),'utf8').replace("'@/lib/explainability'","'../lib/explainability.ts'").replace("'@/components/presentation-dialog'",JSON.stringify(dialogPath.href));
...
unlinkSync(path);unlinkSync(dialogPath);
