...
固定深色主题，顶部显示「近期记录 · 状态分析 · 动态调整」。保留原球体几何与呼吸外层，限制展示尺寸并移除外围 bloom。电脑主页面按视口高度布局；完整决策、连接和调节入口使用原生对话框。移除音频控制行和轻柔按钮，保留系统减少动态效果偏好。时间轴支持前后拖动和键盘定位，本地回放重新建立控制会话以恢复对应状态；实时后端模式禁止拖动。

HTML 使用 scripts/export-inner-light.mjs 直接从源码与构建 CSS 导出，无附加 DOM 改造脚本。1366×768、1440×900、1920×1080 一屏、播放、前后拖动、弹窗已核验，43 项测试及构建通过；未做实时后端设备联调。
