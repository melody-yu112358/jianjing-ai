...
export function useDemoAudio(running:boolean,elapsed:number,demo=true) {
  ...
  const start=useCallback((fresh=false)=>{
    if(!ref.current)ref.current=new DemoAudio(()=>new Audio(),setStatus);
    ...
    if(!narrator.current)narrator.current=new DemoNarrator(speechSynthesis,text=>new SpeechSynthesisUtterance(text),setVoiceStatus);
