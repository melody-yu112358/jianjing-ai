...
import {RoadshowReplay as MindfulnessReplay,roadshowPacket as demoPacket,ROADSHOW_DURATION as DEMO_DURATION,ROADSHOW_PHASES as DEMO_PHASES,ROADSHOW_AUDIO,roadshowDecisionAt} from '@/lib/roadshow-demo';
...
export default function Home(){
 ...
 const sound=useDemoAudio(source==='demo'&&playing,elapsed,source==='demo',ROADSHOW_AUDIO);
 ...
 return <><StarBackdrop playing={active&&!stale} gentle={gentle} sleepProgress={sleepProgress}/><main className={immersive?'app immersive filament-app roadshow-app':'app filament-app roadshow-app'}>
  <header className="topbar"><a className="brand" href="/" aria-label="渐静首页"><Waves size={28}/><strong>渐静</strong><span>JIANJING</span></a><div className="header-note">身心之间，看见变化</div><span className="prototype">{source==='demo'?'模拟案例 · 决策回放':source==='manual'?'手动探索':sourceName}</span></header>
  <div className="workspace">
   ...
   <aside className="insights"><RoadshowInsights elapsed={elapsed} source={source} frame={frame} history={signalHistory} stale={stale} started={!!lastPacket||playing}/>
