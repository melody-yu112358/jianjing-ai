...
export function roadshowSignals(seconds:number){
 ...
 return {heart_rate:v[0]+variation*(Math.sin(t*1.4)*.5+Math.sin(t*.57)*.3),
  resp_rate:v[1]+variation*(Math.sin(t*.89+.6)*.3+Math.sin(t*1.71)*.1),
  arousal:v[2],stability:v[3]};
}
...
export function roadshowPacket(seconds:number,sessionId:string,seq:number,epoch:number):AgentControl{
 const t=Math.max(0,Math.min(60,seconds)),s=roadshowSignals(t),guided=t>=10&&t<20,end=t>=60;
 ...
 return {type:'agent.control',version:'2.0',session_id:sessionId,seq,timestamp:epoch+t,data_source:'simulated',payload:{
  ...
  guidance:{stage:end?'end':t>=54?'fade_out':guided?'guided_breathing':t<10?'assess':t<22?'settling':t<38?'switch_method':'settling',
   text:end?'今晚的睡前仪式已完成。':roadshowGuidanceAt(t)?.text??'可以把屏幕放下了。',inhale_sec:guided?4:0,exhale_sec:guided?6:0}
 }};
}
export class RoadshowReplay{
 ...
 advance(delta:number){
  ...
  const raw=roadshowPacket(this.elapsed,this.sessionId,this.seq++,this.epoch),packet=decodeControl(JSON.stringify(raw));
